...
func get_data(idx: int) -> Dictionary:
	return {"value": _generator.data.variables[variable_name].value}
...
func on_added() -> void:
	...
	var _suffix: int = 1
	while _generator.data.variables.has(variable_name):
		_suffix += 1
		variable_name = "%s%s" % [_line_edit.text, _suffix]
	_line_edit.text = variable_name

	_generator.data.variables[variable_name] = {
		"name": variable_name,
		"type": type,
		"hint": hint,
...
func on_removed() -> void:
	_generator.data.variables.erase(variable_name)
	_generator.data.notify_property_list_changed()


func _on_line_edit_text_changed(new_text: String) -> void:
	_line_edit.remove_theme_color_override("font_color")
	if _generator.data.variables.has(new_text):
		_line_edit.add_theme_color_override("font_color", Color.RED)
		return

	_generator.data.variables[new_text] = _generator.data.variables.get(variable_name)
	_generator.data.variables.erase(variable_name)
	_generator.data.variables[new_text].name = new_text
